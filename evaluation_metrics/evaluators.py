...
def extract_features(model, data_loader, print_freq=50):
    # 对一个数据集的特征进行提取，并记录数据载入时间和特征提取时间
    model.eval()
...
def pairwise_distance(features, query=None, gallery=None):
    # 计算query矩阵和gallery矩阵的距离矩阵
    if query is None and gallery is None:
        n = len(features)
        x = torch.cat(list(features.values())).cpu()
        x = x.view(n, -1)
        dist_m = torch.pow(x, 2).sum(dim=1, keepdim=True) * 2
        dist_m = dist_m.expand(n, n) - 2 * torch.mm(x, x.t())
        return dist_m

    x = torch.cat([features[f].unsqueeze(0) for f, _, _ in query], 0).cpu()
    y = torch.cat([features[f].unsqueeze(0) for f, _, _ in gallery], 0).cpu()
...
def evaluate_all(query_features, gallery_features, distmat, query=None, gallery=None,
                 ...
                 cmc_topk=(1, 5, 10), cmc_flag=False, file='result'):
    # 计算评估指标
    ...
    with open(file + '.txt', 'w') as f:
        f.write('Mean AP: {:4.1%}'.format(mAP) + '\n')
        print('CMC Scores:')
        for k in cmc_topk:
            print('  top-{:<4}{:12.1%}'.format(k, cmc_scores['market1501'][k - 1]))
            f.write('  top-{:<4}{:12.1%}'.format(k, cmc_scores['market1501'][k - 1]) + '\n')
        f.write('--------------------------------------------------------------------' + '\n')
...
class Evaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery, cmc_flag=False, file='result', rerank=False):
